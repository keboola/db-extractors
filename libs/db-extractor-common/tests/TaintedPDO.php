...
class TaintedPDO extends \PDO
{
    ...
    public function query($statement, $mode = null, $arg3 = null, array $ctorargs = [])
    {
        ...
        //fwrite(STDERR, 'querying ' . $statement . PHP_EOL);
        if ($mode !== null) {
...
//        exec('docker network disconnect db-extractor-common_db_network db_tests', $output, $retCode);
        //fwrite(STDERR, 'Output: ' . implode(PHP_EOL, $output) . PHP_EOL);
        //fwrite(STDERR, 'Return: ' . $retCode . PHP_EOL);
        return $ret;
    }
}
